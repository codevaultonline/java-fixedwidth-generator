...
public class Generator {
    ...
    public static void generateFromCsv(File inputFile, File outputDirectory, boolean includesHeader) throws Exception {
        ...
        for (CSVRecord record : records) {
            ...
            String packageName = getValueFromRecord(record, 1);
            String className = getValueFromRecord(record, 2);
            String recordLength = getValueFromRecord(record, 3);
            String classDescription = getValueFromRecord(record, 4);
            String fieldName = getValueFromRecord(record, 5);
            String fieldNameOverride = getValueFromRecord(record, 6);
            String fieldType = getValueFromRecord(record, 7);
            String fieldFormat = getValueFromRecord(record, 8);
            int fieldStartPosition = Integer.parseInt(getValueFromRecord(record, 9));
            int fieldLength = Integer.parseInt(getValueFromRecord(record, 10));
            FieldDefinition.SignedValueIndicator signedValueIndicator = StringUtils.isNotBlank(getValueFromRecord(record, 11)) ? FieldDefinition.SignedValueIndicator.valueOf(getValueFromRecord(record, 11)) : null;
            String additionalInformation = getValueFromRecord(record, 12);
            String extendedInformation = getValueFromRecord(record, 13);
            ...
            // get type
            fieldType = StringUtils.isBlank(fieldType) ? "string" : fieldType.trim().toLowerCase();
            ...
            field.setStartPosition(fieldStartPosition);

            // get field length
            field.setLength(fieldLength);

            // determine field end position
            field.setEndPosition(fieldStartPosition + fieldLength - 1);

            if (!StringUtils.isEmpty(fieldFormat)) {
                if (fieldFormat.matches("^\\d+\\.\\d+$")) {
                    String[] formatParts = fieldFormat.split("\\.");
                    field.setPrecision(Integer.parseInt(formatParts[0]));
                    field.setScale(Integer.parseInt(formatParts[1]));
                } else {
                    field.setFormatString(fieldFormat);
                }
            }
            ...
            if (fieldType.equals("filler")) {
                ...
                // generate field name
                String name = StringUtils.firstNonBlank(fieldNameOverride, fieldName).toLowerCase();
                ...
                StringBuilder standardizedFieldName = new StringBuilder();

                for (int i = 0; i < fieldNameParts.length; i++) {
                    if (i == 0) {
                        standardizedFieldName.append(fieldNameParts[i]);
                    } else {
                        standardizedFieldName.append(StringUtils.capitalize(fieldNameParts[i]));
                    }
                }

                field.setFieldName(standardizedFieldName.toString());
            ...
            for (FieldDefinition field : record.getFields()) {
                ...
                if (field.isFiller()) {
                    ...
                    if (null != field.getPrecision()) {
                        methodAnnotation.addMember("precision", "$L", field.getPrecision());
                    }

                    if (null != field.getScale()) {
                        methodAnnotation.addMember("scale", "$L", field.getScale());
                    }
    ...
    private static String getValueFromRecord(CSVRecord record, int columnNumber) {

        if (record.size() >= columnNumber) {
            return record.get(columnNumber - 1);
        }
